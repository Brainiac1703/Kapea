...
public class PlatformAccountTests
{
    ...
    {
        var account = PlatformAccount.Create(Owner, PlatformCode.Kraken, "  Kraken principal  ", Currency.Euro);

        Assert.Equal(PlatformCode.Kraken, account.Platform);
    ...
    public void An_account_without_alias_is_rejected() =>
        Assert.Throws<DomainException>(() => PlatformAccount.Create(Owner, PlatformCode.Xtb, "   ", Currency.Euro));

    [Fact]
    public void Deleting_an_account_with_transactions_is_rejected()
    {
        var account = PlatformAccount.Create(Owner, PlatformCode.Xtb, "XTB", Currency.Euro);
